...
トンネルデバイス（`ip6tnl0`）を作成し、その上にトンネルを張る。

```systemd title="/etc/systemd/network/05-ip6tnl0.netdev"
[NetDev]
Name=ip6tnl0
Kind=ip6tnl
...
```systemd title="/etc/systemd/network/05-ip6tnl0.network"
[Match]
Name=ip6tnl0
...
あとで`tc`のフィルタを付けるため、この時点でベースとなるqdiscも設定している[^1]。

[^1]: ちなみに`clsact`というのは、それ自体は特に何もしない（フィルタなどをアタッチするためだけ）のqdiscで、ingressにもegressにも使えるらしい

なお`[Route]` セクションの代わりに、`Network.DefaultRouteOnDevice=yes` を指定することもできる。細かい設定（`Route.InitialCongestionWindow`など）が不要な場合、そちらのほうが楽だろう。

#### MAP-E関連の設定

[前回紹介した記事](https://turgenev.hatenablog.com/entry/2024/04/23/031222)に倣い、`tc`を使ったNATなどを設定する。iptablesの代わりにnftablesを使い、TCP MSSの自動調整（MSS Clamping）を加えている点以外は、リンク先とほぼ同じだ[^2]。

[^2]: とはいえ少しだけ改変している。主な差異は以下の通り：1. tcのqdiscとして`clsact`を使っている / 2. qdiscはsystemd-networkdですでに追加済み / 3. fwmarkの下位2桁以外を無視している / 4. `traceroute`に必要な、ICMP Time Exceeded（11）を処理している

まずMSS Clamping・fwmarkの付与・ソースNATをするnftablesのルールを書く。

```NFTables title="/etc/nftables.d/05-ip6tnl0.nft"
#!/usr/bin/nft -f
...
起動時に実行すべき[^3]`tc`コマンドも、スクリプトにまとめておく。

[^3]: `tc`コマンドの効果は永続しないため、再起動ごとに実行する必要がある

```sh title="/usr/local/sbin/tc-ip6tnl0.sh"
#!/usr/bin/env bash
...
このスクリプトを、`ip6tnl0`デバイスの存在が確認された時点で適用するよう、systemdサービスを設定する。

```systemd title="/etc/systemd/system/tc-ip6tnl0.service"
[Unit]
...
続いて宅内機器側のインターフェース（`home`）を設定する。IPv6はSLAAC + RDNSS、IPv4はDHCPv4でそれぞれ配布している。

...
ところがひかり電話を使っている場合、何も考えずエイヤとHGWを取り外してはいけない。というのも、HGWは**ひかり電話ルータ**でもあり、まともに機能するには以下の条件が必要だからだ。

1. DHCPv4でフレッツ網から**グローバルIPv4アドレス**（/30）を受け取ること
2. DHCPv6オプション（**ひかり電話関連情報**）を受け取ること
...
しかしLinuxでルータを作れば、これらすべて──IPv4のブリッジ、DHCPv6オプションの受信、DHCPv6サーバ──を一台で担える。そのうえ適当な端末（たとえばミニPC）さえあれば作れる。ちょっとしたDIYで、シンプルかつ多機能な構成を実現できるのだ。
...
以下では例示のため、プロバイダから割り振られたIPv6プレフィックスを`3fff:0:0:ab00::/56`とする。IPv4 over IPv6（MAP-E）の接続に必要な情報は、あらかじめプロバイダから得るか、さもなくば[計算機](https://ipv4.web.fc2.com/map-e.html)などで算出しておく。ここでは下表のように仮定する。

| 項目                      | 値                     |
| :---------------------- | :-------------------- |
| CEアドレス                  | `3fff:0:0:ab00::ce`   |
| IPv4アドレス                | `192.0.2.1`           |
| ポートセット（PS）ID            | `171`（10進）/ `ab`（16進） |
| BRアドレス（option peeraddr） | `2001:db8::1/64`      |
...
ルータ用の機材には、適当なミニPCを使った。ミニPCだけではポート数が足りないため、タグVLANに対応したL2スイッチも用意した。
...
ブリッジデバイスを使ってもよいが、nftablesの[fwd文](https://www.netfilter.org/projects/nftables/manpage.html#:~:text=FWD%20STATEMENT)が便利だ。入ってくる生のパケットをそのまま別のインターフェースに転送できるので、HGW側から見ると（IPv4では）実質L2スイッチのような振る舞いになる。
...
理想としては、フレッツ網から受け取ったDHCPv6オプションをそのままHGWに「中継」できればよいが、これができるソフトウェアはいまのところない。そこで代わりに、**DHCPv6のリースを受けるたびに、リース内容をもとに手元のDHCPv6サーバの設定を更新する**仕組みを作る。

鍵となるのは、systemd-networkdの[D-Bus API](https://www.freedesktop.org/software/systemd/man/latest/org.freedesktop.network1.html)と、Kea（今回使うDHCPv6サーバ）の[Management API](https://kea.readthedocs.io/en/stable/arm/ctrl-channel.html)だ。systemd-networkdは、リースを受けるたびにD-Busシグナルを飛ばしてくれるうえ、リースの内容自体もD-Busで取得できる。一方、KeaのManagement APIを使うと、HTTPで動的に設定を投入できる。両者を合わせると[^6]、以下のような処理が可能になる。

[^6]: systemd-networkdには[DHCPv6サーバ機能がない](https://github.com/systemd/systemd/issues/16039)ため、併用が必要

1. DHCPv6リースを受けた際のD-Busシグナルを検知
2. 検知したら、D-Busを叩き、リース内容を取得
3. リース内容を元に、KeaのAPIを叩いて設定を更新

この処理を適当な言語で実装[^7]すれば、仮に将来フレッツ網から降ってくる情報が変わっても対応できる。

[^7]: 今回はPythonで実装した。こんな処理をわざわざコンパイルするのも気が引けるし、かといってシェルスクリプトには荷が重いからだ。D-Busとのやり取りには、[python-sdbus](https://github.com/python-sdbus/python-sdbus)というライブラリが便利だ。
...
router.wan0 <-> router.ip6tnl0: カプセル処理 {class: ipv4; style.stroke-dash: 3}
router.ip6tnl0 -- router.lan0.home: NAT +\nポート変換（tc） {class: ipv4}
...
- `ip6tnl0@wan0`：IPv4 over IPv6のトンネルデバイス
- `home@lan0`：VLAN用サブインターフェース（ID = 10）
  - 宅内機器側セグメントのデフォルトゲートウェイ
- `hgw@lan0`：VLAN用サブインターフェース（ID = 100）
  - HGW側セグメントのデフォルトゲートウェイ（IPv6のみ）

L2スイッチ側でも、これに対応したタグVLAN（ID = 10, 100）を設定する[^8]。

[^8]: なお、もう一つVLANを切ってHGWの**LAN側**と接続しておくと、ルータ経由でHGWのWeb設定画面にアクセスできて便利だが、今回は省略した
...
今回使った機材には、RTL8125BというRealtek :crab: の2.5GbE NICが載っていた。しかし初期状態では汎用のドライバ（`r8169`）が当たっており、本来の機能を発揮できない。そこで製造元から配られている専用ドライバをインストールする。Arch Linuxの場合、[AURにあるパッケージ](https://aur.archlinux.org/packages/r8125-dkms)を使うと楽だ。

デフォルトのビルド設定はかなり保守的になっていて、たとえばマルチキューなどの機能が有効化されていない。そのため、適宜書き換えてビルドする[^9]。

[^9]: OpenWrtでは[デフォルトでマルチキューが有効化されている](https://github.com/openwrt/openwrt/pull/18573)ため、それに倣った
...
ルータとして使う以上、IPv4 / IPv6の転送を許可する設定は必須だ。
...
あとでIPv4 over IPv6トンネルを張る際に、勝手にトンネルデバイス（`ip6tnl0@none`）が作成されないような設定もしておく。

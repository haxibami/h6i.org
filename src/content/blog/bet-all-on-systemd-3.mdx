...
[ネットワーク構成](./bet-all-on-systemd-1#ネットワーク構成)でみた通り、HGWに対してはDHCPv6-PDでプレフィックスを配る必要がある。このとき、DHCPv6サーバ自体はKeaに任せるとしても、RAによるデフォルトゲートウェイの通知[^1]が別途必要になる。この部分については、systemd-networkdだけで実現できる。

[^1]: DHCPv6ではデフォルトゲートウェイを通知できないため

RAのパラメータは、（HGWが本来受け取る）フレッツ網のものを参考にして決めるとよい。
...
[Service]
Type=oneshot
ExecStart=/usr/sbin/nft -f /etc/nftables.d/lazy/hgw.nft
...
ちなみに`ingress`フックは対象デバイスが存在しないとエラーになる。そのため上のようにsystemdサービスなどを利用し、デバイスの存在を保証してから適用する必要がある。
...
+    udp dport dhcpv6-server ip6 saddr fe80::/10 iifname "hgw" accept
...
+    iifname { "home", "hgw" } oif "wan0" accept comment "v4"
...
+    meta protocol ip6 iifname { "home", "hgw" } counter accept
...
DHCPv6サーバ（Kea）は、下のような最低限の設定で動作させる。

払い出すIPv6プレフィックスや、DHCPv6オプションの内容はここには書かない。[あとで書く処理](#ひかり電話関連情報の配布)で動的に追加するからだ。
...
ただし、HGWにプレフィックスを払い出す際には、払い出したプレフィックスに向けて経路を設定する必要がある（逆に、失効時は削除が必要）。このため[フック](https://kea.readthedocs.io/en/stable/arm/hooks.html#libdhcp-run-script-so-run-script-support-for-external-hook-scripts)を使い、払い出し・失効などのタイミングで以下のスクリプトを実行している。
...
[第二部](./bet-all-on-systemd-2#ipv6-ipoe)のように、ただDHCPv6クライアントを動かしただけでは、ひかり電話の関連情報は降ってこない。種明かしをしてしまうと、DHCPv6リクエストで
...
必要なHGWのMACアドレス（`de:ad:be:ef:00:05`と仮定）は、ルータで
...
スクリプト内にも書いたが、執筆時点でstable版のsystemd（v257）では、IPv6のSIPサーバアドレスを取得できない[^7]。そのためv258のリリースを待つか、手動でキャプチャ[^8]した内容をハードコードするほかない。

[^7]: 仕方がないのでこのために[コントリビュートした](https://github.com/systemd/systemd/pull/38269)

[^8]: `tshark -i wan0 -f "udp src port 547" -Y "dhcpv6.msgtype == 7 and dhcpv6.sip_server_a" -T ek -e dhcpv6.sip_server_a`
